...
public class EnchantedBookLogic {
    ...
    public void updateEnchantingStack() {
        ItemStack itemStack = this.getEnchantingStack();
        ServerPlayerEntity mp = (ServerPlayerEntity) this.container.getPlayer();
        boolean flag = true;
        if (itemStack.getItem() instanceof EnchantedBookItem && this.enchantments.size() == 0) {
            ...
            mp.connection.send(new SSetSlotPacket(this.container.containerId, 0, ItemStack.EMPTY));
        }

        if (flag) {
            EnchantmentUtil.setEnchantedItemStack(this.enchantments, itemStack);
            mp.connection.send(new SSetSlotPacket(this.container.containerId, 0, itemStack));
        }
    ...
    public void updateBookStack() {
        ...
        ServerPlayerEntity mp = (ServerPlayerEntity) this.container.getPlayer();
        EnchantmentUtil.setEnchantedItemStack(this.enchantments, itemStack);
        mp.connection.send(new SSetSlotPacket(this.container.containerId, 1, itemStack));
    }
    ...
    public void updateBookArrayStack(boolean flag) {
        ...
        if (length != 0) {
            for (int i = 0; i < length; ++i) {
                ...
                slotId = i + 2;
                mp.connection.send(new SSetSlotPacket(windowId, slotId, itemStack));
            }
        }
    ...
    public void clearBookArrayStack() {
        ...
        for (int i = 0; i < length; ++i) {
            if (!inventory.getStackInSlot(i).isEmpty() || i == length - 1) {
                ...
                int slotId = i + 2;
                mp.connection.send(new SSetSlotPacket(windowId, slotId, ItemStack.EMPTY));
            }
        }
