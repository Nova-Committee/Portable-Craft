...
public class AnvilScreen extends AbstractRepairScreen<AnvilContainer> {
    ...
    public AnvilScreen(AnvilContainer handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY) {
        ...
        int i = ((AnvilContainer) this.handler).getLevelCost();
        if (i > 0) {
            Text text;
            int j = 8453920;
            if (i >= 40 && !this.client.player.getAbilities().creativeMode) {
                ...
                j = 0xFF6060;
            } else if (!((AnvilContainer) this.handler).getSlot(2).hasStack()) {
                text = null;
            } else {
                text = new TranslatableText("container.repair.cost", i);
                if (!((AnvilContainer) this.handler).getSlot(2).canTakeItems(this.player)) {
                    j = 0xFF6060;
                }
            }
            if (text != null) {
                ...
                int l = 69;
                net.minecraft.client.gui.screen.ingame.AnvilScreen.fill(matrices, k - 2, 67, this.backgroundWidth - 8, 79, 0x4F000000);
