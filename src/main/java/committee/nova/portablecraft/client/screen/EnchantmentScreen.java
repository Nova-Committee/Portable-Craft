...
public class EnchantmentScreen extends HandledScreen<EnchantmentContainer> {
    private static final Identifier TEXTURE = new Identifier(Portablecraft.MOD_ID + ":textures/gui/container/enchanting_table.png");
    ...
    public EnchantmentScreen(EnchantmentContainer handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
        EnchantingPhrases.getInstance().setSeed(this.handler.getEnchantmentSeed());
        int n = this.handler.getGoldCount();
        for (int o = 0; o < 3; ++o) {
            int p = i + 60;
            int q = p + 20;
            this.setZOffset(0);
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int i = this.handler.getGoldCount();
        for (int j = 0; j < 3; ++j) {
            ...
            if (!bl) {
                ...
                if (this.client.player.experienceLevel < k) {
                    list.add(new TranslatableText("container.enchant.level.requirement", ((EnchantmentContainer) this.handler).enchantmentPower[j]).formatted(Formatting.RED));
                } else {
